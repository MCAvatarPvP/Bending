...
public class CoreMethods {
    ...
    public static BlockIterator blockRayTrace(Block origin, Block target) {
        ...
        if (direction.lengthSquared() == 0) {
            return new BlockIterator(origin.getWorld(), originVector, new Vector(0, 1, 0), 0, 0);
        }
    ...
    public static BlockData getWallBlockData(Player player, boolean earthOnly) {
        for (BlockFace face : faces) {
            Block block = player.getLocation().getBlock().getRelative(face);
            if (GeneralMethods.isSolid(block) && !block.getType().equals(Material.BARRIER)) {
                if (earthOnly && !EarthAbility.isEarthbendable(player, block)) continue;
    ...
    public static Vector calculateFlatVector(Location start, Location end) {
        Vector vector = new Vector(end.getX() - start.getX(), 0, end.getZ() - start.getZ());
    ...
    public static void loadAbilities() {
        ...
        if (ConfigManager.getConfig().getBoolean("EnableCollisions")) setupCollisions();
    }
    ...
    public static void setAttributes(PKConfigurationSection section, CoreAbility ability) {
