...
public class BloodGrip extends BloodAbility implements AddonAbility, MultiAbility {
    ...
    public BloodGrip(Player player, boolean sneak) {
        ...
        if (!MultiAbilityManager.hasMultiAbilityBound(player, getName()) && sneak) {
            range = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.Range");
            Entity e = GeneralMethods.getTargetedEntity(player, range);

            if (e == null || !(e instanceof LivingEntity)) {
                return;
            } else if (ProjectAddons.instance.getConfig(bPlayer).getStringList("Abilities.Water.BloodGrip.EntityFilter").contains(e.getType().toString())) {
                return;
            }

            target = (LivingEntity) e;
            cooldown = ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.BloodGrip.Cooldown");
            dragSpeed = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.DragSpeed");
            throwPower = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.ThrowPower");
            mangleDmg = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.MangleDamage");
            slamSpeed = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.SlamSpeed");

            health = player.getHealth() - ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.BloodGrip.DamageThreshold");
            TARGETS.add(target);
    ...
    public void puppetAttack() {
        for (Entity e : GeneralMethods.getEntitiesAroundPoint(target.getLocation(), 1)) {
            if (e instanceof LivingEntity && !e.equals(target) && !e.equals(player)) {
    ...
    public boolean isEnabled() {
        //return ProjectAddons.instance.getConfig(bPlayer).getBoolean("Abilities.Water.BloodGrip.Enabled");
        return false;
    }
}
