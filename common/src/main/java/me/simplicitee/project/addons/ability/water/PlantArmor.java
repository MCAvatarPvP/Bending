...
public class PlantArmor extends PlantAbility implements AddonAbility, MultiAbility {
    ...
    public PlantArmor(Player player, ClickType type) {
        ...
        if (type == ClickType.SHIFT_DOWN) {
            this.duration = ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.PlantArmor.Duration");
            this.cooldown = ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.PlantArmor.Cooldown");
            this.durability = this.maxDurability = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.Durability");
            this.duration = ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.PlantArmor.Duration");
            this.durabilityDecay = duration <= 0 ? 0 : maxDurability / (20.0 * duration / 1000.0);
            this.swim = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.Boost.Swim") - 1;
            this.speed = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.Boost.Speed") - 1;
            this.jump = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.Boost.Jump") - 1;
            this.armor = null;
            ...
            this.requiredPlants = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.RequiredPlants");
            this.selectRange = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SelectRange");

            this.forward = true;
            this.range = 0;
            this.maxRange = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.VineWhip.Range");
            this.vdmg = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SubAbilities.VineWhip.Damage");
            this.vSpeed = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.VineWhip.Speed");

            this.radius = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.LeafShield.Radius");

            this.current = null;
            this.tRadius = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SubAbilities.Tangle.Radius");
            this.tRange = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SubAbilities.Tangle.Range");
            this.tDuration = ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.PlantArmor.SubAbilities.Tangle.Duration");
            this.angle = 0;

            this.gMax = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.Grapple.Range");
            this.gSpeed = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SubAbilities.Grapple.Speed");
            this.pulling = false;
            this.gRange = 0;

            this.dRadius = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.LeafDome.Radius");

            this.power = ProjectAddons.instance.getConfig(bPlayer).getDouble("Abilities.Water.PlantArmor.SubAbilities.Leap.Power");

            this.regen = ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities.Regenerate.RegenAmount");
    ...
    private void reset() {
        if (active != null) {
            bPlayer.addCooldown(active.getName(), ProjectAddons.instance.getConfig(bPlayer).getLong("Abilities.Water.PlantArmor.SubAbilities." + active.getName() + ".Cooldown"));
    ...
    private void progressForming() {
        if (sources.size() == requiredPlants) {
            ...
            this.bar.setTitle(ChatColor.DARK_AQUA + "Durability [" + ChatColor.GREEN + durability + ChatColor.DARK_AQUA + " / " + maxDurability + "]");
    ...
    private void progressTangle() {
        ...
        for (Entity e : GeneralMethods.getEntitiesAroundPoint(current, tRadius + 0.5)) {
            if (e instanceof LivingEntity && e.getEntityId() != player.getEntityId()) {
                new MovementHandler((LivingEntity) e, this).stopWithDuration(tDuration, ChatColor.DARK_AQUA + "* Tangled *");
    ...
    private int getAbilityCost(String ability) {
        return ProjectAddons.instance.getConfig(bPlayer).getInt("Abilities.Water.PlantArmor.SubAbilities." + ability + ".Cost");
    }
    ...
    public boolean isEnabled() {
        return ProjectAddons.instance.getConfig(bPlayer).getBoolean("Abilities.Water.PlantArmor.Enabled");
    }
    ...
    public static enum ArmorAbility {
        ...
        LEAP("Leap", true, ClickType.LEFT_CLICK, (player -> GeneralMethods.isOnGround(player))),
