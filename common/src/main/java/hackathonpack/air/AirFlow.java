...
public class AirFlow extends AbstractAirFlow implements AddonAbility, ComboAbility {
    private long cooldown;

    public AirFlow(final Player player) {
        super(player);
    ...
    private static String path(final String node) {
        return "ExtraAbilities.Hiro3.HackathonPack.Air.AirFlow." + node;
    }
    ...
    public Object createNewComboInstance(final Player player) {
        return new AirFlow(player);
    }

    @Override
    public ArrayList<AbilityInformation> getCombination() {
        return UtilityMethods.getConfiguredCombination(path("Enable"), path("Combination"), new ArrayList<>(Arrays.asList(new AbilityInformation("AirBurst", ClickType.SHIFT_DOWN), new AbilityInformation("AirBlast", ClickType.LEFT_CLICK))));
    }
    ...
    public void load() {
        ...
        ConfigManager.getConfig().addDefault(path("Combination"), Arrays.asList("AirBurst: SHIFT_DOWN", "AirBlast: LEFT_CLICK"));
