...
public class FireBall extends FireAbility implements AddonAbility {
    ...
    public void setFields() {
        range = JedCoreConfig.getConfig(this.bPlayer).getLong("Abilities.Fire.FireBall.Range");
        fireTicks = JedCoreConfig.getConfig(this.bPlayer).getLong("Abilities.Fire.FireBall.FireDuration");
        cooldown = JedCoreConfig.getConfig(this.bPlayer).getLong("Abilities.Fire.FireBall.Cooldown");
        damage = JedCoreConfig.getConfig(this.bPlayer).getDouble("Abilities.Fire.FireBall.Damage");
        forwardSpeed = JedCoreConfig.getConfig(this.bPlayer).getDouble("Abilities.Fire.FireBall.ForwardSpeed");
        sideSpeed = JedCoreConfig.getConfig(this.bPlayer).getDouble("Abilities.Fire.FireBall.SideSpeed");
        controllable = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Fire.FireBall.Controllable");
        fireTrail = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Fire.FireBall.FireTrail");
        collisionRadius = JedCoreConfig.getConfig(this.bPlayer).getDouble("Abilities.Fire.FireBall.CollisionRadius");
    ...
    private void progressFireball() {
        for (int i = 0; i < 2; i++) {
            ...
            if (controllable) {
                ...
                location.add(sideDir);
                direction = playerLoc.getDirection().multiply(forwardSpeed);
            }
            ...
            boolean hitTarget = CollisionDetector.checkEntityCollisions(player, new Sphere(location, collisionRadius), this::doDamage);
    ...
    public void handleCollision(Collision collision) {
        if (collision.isRemovingFirst()) {
            ...
            if (second instanceof AirShield) {
                ...
                boolean reflect = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Fire.FireBall.Collisions.AirShield.Reflect", true);
    ...
    public String getDescription() {
        ...
        return "* JedCore Addon *\n" + JedCoreConfig.getConfig(this.bPlayer).getString("Abilities.Fire.FireBall.Description");
    }
    ...
    public boolean isEnabled() {
        ...
        return JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Fire.FireBall.Enabled");
    }
}
