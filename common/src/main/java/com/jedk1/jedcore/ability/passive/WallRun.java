...
public class WallRun extends ChiAbility implements AddonAbility, PassiveAbility {
    ...
    private boolean chi;
    private boolean cancelWhenSneak;
    ...
    public void setFields() {
        enabled = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Enabled");
        cooldown = JedCoreConfig.getConfig(this.bPlayer).getLong("Abilities.Passives.WallRun.Cooldown");
        duration = JedCoreConfig.getConfig(this.bPlayer).getLong("Abilities.Passives.WallRun.Duration");
        particles = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Particles");
        air = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Air");
        earth = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Earth");
        water = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Water");
        fire = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Fire");
        chi = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Chi");
        cancelWhenSneak = JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.CancelWhenSneak");
        invalid = JedCoreConfig.getConfig(this.bPlayer).getStringList("Abilities.Passives.WallRun.InvalidBlocks");
    }
    ...
    private boolean isEligible() {
        ...
        if (player.isSneaking() && cancelWhenSneak) return false;
        ...
        if (bPlayer.getElements().contains(Element.AIR) && air) {
            ...
            return (bPlayer.getElements().contains(Element.CHI) || bPlayer.getElements().contains(Element.getElement("MartialArts"))) && chi;
    }
    ...
    private boolean isAgainstWall() {
        ...
        if (location.getBlock().getRelative(BlockFace.NORTH).getType().isSolid() && !invalid.contains(location.getBlock().getRelative(BlockFace.NORTH).getType().name())) {
            ...
            return location.getBlock().getRelative(BlockFace.EAST).getType().isSolid() && !invalid.contains(location.getBlock().getRelative(BlockFace.EAST).getType().name());
    }

    @Override
    public void progress() {
        if (player.isDead() || !player.isOnline() || GeneralMethods.isOnGround(player)) {
            remove();
            return;
        }
        if (!isAgainstWall()) {
        ...
        if (player.isSneaking() && cancelWhenSneak) {
            remove();
            return;
        }
    ...
    public boolean isEnabled() {
        return JedCoreConfig.getConfig(this.bPlayer).getBoolean("Abilities.Passives.WallRun.Enabled");
    }
