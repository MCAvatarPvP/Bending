...
public class BendingBoard {
    ...
    static {
        toggled = new Config(new File("JedCore/board/players.yml"));
    ...
    public static void loadOtherCooldowns() {
        PKConfigurationSection section = JedCoreConfig.board.getConfig().getConfigurationSection("Settings.OtherCooldowns");

        otherAbilities.clear();

        for (String ability : section.getKeys(false)) {
            PKConfigurationSection abilitySection = section.getConfigurationSection(ability);
    ...
    public static void updateOnline() {
        if (!enabled) return;
        for (Player player : Platform.players().onlinePlayers().stream().map(Player.class::cast).toList()) {
    ...
    public void remove() {
        ...
        player.setScoreboard(Platform.scoreboards().mainScoreboard());
    }
