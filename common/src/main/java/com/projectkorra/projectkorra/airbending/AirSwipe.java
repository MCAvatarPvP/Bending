...
public class AirSwipe extends AirAbility {
    ...
    private double forwardVelocityFactor;
    private double backwardVelocityFactor;
    ...
    public AirSwipe(final Player player, final boolean charging) {
        ...
        this.forwardVelocityFactor = Math.max(0, getConfig().getDouble("Abilities.Air.AirSwipe.PlayerVelocity.ForwardFactor", 1.0));
        this.backwardVelocityFactor = Math.max(0, getConfig().getDouble("Abilities.Air.AirSwipe.PlayerVelocity.BackwardFactor", 1.0));
    ...
    private void advanceSwipe() {
        for (final Vector direction : this.streams.keySet()) {
            ...
            if (direction != null && location != null) {
                final double currentSpeed = this.getSwipeSpeed(direction);

                BlockIterator blocks = new BlockIterator(this.getLocation().getWorld(), location.toVector(), direction, 0, (int) Math.ceil(currentSpeed));
                ...
                location = location.clone().add(direction.clone().multiply(currentSpeed));
    ...
    private double getSwipeSpeed(final Vector direction) {
        if (this.launchVelocity == null) {
            return this.speed;
        }

        final double projectedVelocity = this.launchVelocity.dot(direction);
        final double velocityFactor = projectedVelocity >= 0 ? this.forwardVelocityFactor : this.backwardVelocityFactor;
    ...
    private boolean shouldSkipKnockback(final Entity entity) {
        return entity instanceof Player && Catapult.isAwaitingLanding((Player) entity);
    }
    ...
    private void launch() {
        ...
        this.launchVelocity = this.player.getVelocity().clone();
        for (double i = -this.arc; i <= this.arc; i += this.arcIncrement) {
            final double angle = Math.toRadians(90 + i);
            ...
            this.streams.put(vec, this.origin);
        }
    }
