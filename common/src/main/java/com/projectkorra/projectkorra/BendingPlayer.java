...
public class BendingPlayer extends OfflineBendingPlayer {
    ...
    private double lastDisplayedAirStamina = Double.NaN;
    private long airStaminaDisplayUntil;
    private double surgeWaveDecay;
    private long lastSurgeWaveTime;
    ...
    public BendingPlayer(Player player) {
        ...
        this.surgeWaveDecay = 1;
        super.setCurrentSlot(player.getInventory().getHeldItemSlot());
    }
    ...
    public static BendingPlayer getBendingPlayer(final String playerName) {
        ...
        final Player player = Platform.players().getPlayer(playerName);
        final OfflinePlayer oPlayer = player != null ? Platform.players().getOfflinePlayer(player.getUniqueId()) : null;
    ...
    public static OfflineBendingPlayer getOfflineBendingPlayer(final String playerName) {
        ...
        final OfflinePlayer oPlayer = Platform.players().getOfflinePlayer(playerName);
    ...
    public static boolean isGeyserPlayer(final Player player) {
        return BedrockUtil.isBedrockPlayer(player.getUniqueId());
    }
    ...
    private boolean canBend(@NotNull final CoreAbility ability, final boolean ignoreBinds, final boolean ignoreCooldowns) {
        ...
        if (!this.player.isOnline() || this.player.isDead() || this.player.getGameMode() == GameMode.SPECTATOR) {
            return false;
        }
    ...
    private PKConfiguration getConfig() {
    ...
    public void removeCooldown(final String ability) {
        ...
        if (!event.isCancelled()) {
            ...
            CooldownSync.removed(this, ability);
    ...
    public void recalculateTempElements(boolean wasOffline) {
        ...
        while (subIterator.hasNext() && (subElement = subIterator.next()) != null) {
            ...
            if (time == -1L)
                continue; //The subelement expiry is connected to the parent element, so skip it as it is handled bellow
        ...
        while (elementIterator.hasNext() && (element = elementIterator.next()) != null) {
            ...
            if (System.currentTimeMillis() >= time) {
                ...
                if (element == Element.AVATAR) {
                    ...
                    SubElement s1;
                    while (subIterator1.hasNext() && (s1 = subIterator1.next()) != null) {
                        if (this.tempSubElements.get(s1) != -1L)
                            continue; //Only remove if the subelement is connected to the parent element's time
    ...
    public void removeUnusableAbilities() {
        ...
        for (final int i : slots.keySet()) {
            final CoreAbility boundAbility = CoreAbility.getAbility(slots.get(i));
            if (this.canBind(boundAbility) || boundAbility instanceof AddonAbility) {
    ...
    protected void postLoad() {
        this.bedrock = isGeyserPlayer(this.player);
    ...
    public void fixSubelements() {
        ...
        for (Element tempElement : this.tempElements.keySet()) {
            if (tempElement instanceof SubElement)
    ...
    public void increaseAirBlastDecay(final double amount, final double minimum) {
        this.airBlastDecay = Math.max(minimum, this.airBlastDecay - amount);
    }

    public void regenerateAirBlastDecay(final double amount, final double maximum) {
        this.airBlastDecay = Math.min(maximum, this.airBlastDecay + amount);
    }
    ...
    public void setAirBlastDecay(final double value) {
        this.airBlastDecay = value;
    }

    /**
     * Tracks stamina changes for transient action-bar rendering.
     *
    ...
    public boolean shouldDisplayChangingAirStamina(final long displayDuration) {
        ...
        if (Math.abs(this.airBlastDecay - this.lastDisplayedAirStamina) > 0.0001) {
            this.lastDisplayedAirStamina = this.airBlastDecay;
            this.airStaminaDisplayUntil = now + Math.max(0, displayDuration);
        }
        return now < this.airStaminaDisplayUntil;
    }
    ...
    public void increaseSurgeWaveDecay(final double amount, final double minimum) {
        surgeWaveDecay = Math.max(minimum, surgeWaveDecay - amount);
    }

    public void resetSurgeWaveDecay(final double maximum) {
        surgeWaveDecay = maximum;
    }

    public double getSurgeWaveDecay() {
        return surgeWaveDecay;
    }
