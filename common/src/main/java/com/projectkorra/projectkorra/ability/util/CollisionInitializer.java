...
public class CollisionInitializer {
    ...
    public void initializeDefaultCollisions() {
        final CoreAbility airWheel = CoreAbility.getAbility("AirWheel");
        ...
        ProjectKorra.plugin.getServer().getScheduler().runTaskLater(ProjectKorra.plugin, () -> {
            PKConfiguration collisionConfig = ConfigManager.collisionConfig.get();

            for (String s : collisionConfig.getStringList("Collisions")) {
                String[] names = s.split("\\s*,\\s*");
                if (names.length != 2 || names[0].equalsIgnoreCase("FirstAbility")) continue;
                CoreAbility first = CoreAbility.getAbilityByClassName(names[0].trim());
                CoreAbility second = CoreAbility.getAbilityByClassName(names[1].trim());
                if (first == null || second == null) {
                    ProjectKorra.log.warning("Couldn't find collision ability in: " + s);
                    continue;
                }
                // Idempotent for reversed/duplicate entries and persistent for
                // collisions registered later by embedded or external addons.
                this.collisionManager.disableCollision(first, second);
            }

            for (String s : collisionConfig.getStringList("AddCollisions")) {
                String[] abils = s.split("\\s*,\\s*");
                if (abils.length == 4) {
                    if (abils[0].equalsIgnoreCase("FirstAbility")) continue;
                    CoreAbility first = CoreAbility.getAbilityByClassName(abils[0].trim());
                    CoreAbility second = CoreAbility.getAbilityByClassName(abils[1].trim());
                    ...
                    this.collisionManager.enableCollision(first, second);
