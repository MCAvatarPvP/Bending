...
public abstract class FireAbility extends ElementalAbility {
    ...
    public static boolean canGoThroughWater() {
        return ConfigManager.getConfig().getBoolean("Properties.Fire.CanGoThroughTempWater");
    }

    public static boolean canPassThroughWater(final Block block) {
        return canGoThroughWater()
                && isWater(block)
    ...
    public static void playFirebendingParticles(BendingPlayer bPlayer, final Location loc, final int amount, final double xOffset, final double yOffset, final double zOffset, final double speed) {
        ...
        boolean hasCosmetic = bPlayer.getFireColor() != null && !bPlayer.getFireColor().getName().equalsIgnoreCase("none");
        if (hasCosmetic) {
            if (bPlayer.getFireColor().usesNativeParticle()) {
                ParticleUtil.spawn(bPlayer.getFireColor().getParticle(), loc, amount, xOffset, yOffset, zOffset, speed);
                return;
            }
    ...
    public static void playLightningbendingParticle(final Location loc, final double xOffset, final double yOffset, final double zOffset) {
        ParticleEffect.ELECTRIC_SPARK.display(loc, 1, xOffset, yOffset, zOffset);
        GeneralMethods.displayColoredParticle("#01E1FF", loc, 3, xOffset, yOffset, zOffset);
    }
    ...
    public static void playLightningbendingHitSound(final Location loc) {
        if (ConfigManager.getConfig().getBoolean("Properties.Fire.PlaySound")) {
            ...
            try {
                sound = Sound.valueOf(ConfigManager.getConfig().getString("Properties.Fire.LightningHit.Sound"));
            } catch (final IllegalArgumentException exception) {
                ProjectKorra.log.warning("Your current value for 'Properties.Fire.LightningHit.Sound' is not valid.");
            } finally {
                loc.getWorld().playSound(loc, sound, 15, pitch);
            }
    ...
    public void createTempFire(final Location loc, final long time) {
        if (isIgnitable(loc.getBlock())) {
            new TempBlock(loc.getBlock(), createFireState(loc.getBlock(), getFireType() == Material.SOUL_FIRE), time, this);
