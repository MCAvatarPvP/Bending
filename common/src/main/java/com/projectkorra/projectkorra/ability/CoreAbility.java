...
public abstract class CoreAbility implements Ability {
        ...
        for (final Set<CoreAbility> setAbils : INSTANCES_BY_CLASS.values()) {
            for (final CoreAbility abil : setAbils) {
                if (abil instanceof PassiveAbility) {
                    ...
                    if (abil.getPlayer() != null && !abil.getPlayer().isOnline()) { // This has to be before isDead as isDead.
                        abil.remove(); // will return true if they are offline.
                        continue;
                    } else if (abil.getPlayer() != null && abil.getPlayer().isDead()) {
                        continue;
                    }
                } else if (abil.getPlayer() != null && abil.getPlayer().isDead()) {
                    abil.remove();
                    continue;
                } else if (abil.getPlayer() != null && !abil.getPlayer().isOnline()) {
                    abil.remove();
                    continue;
                }
                ...
                try {
                    ...
                    AbilityExecutionContext.run(abil, abil::progress);
                    //}
                    ...
                    try {
                        ...
                        Platform.logger().severe("unable to notify ability user of error");
                    }
                    try {
                        abil.remove();
    ...
    public static CoreAbility getAbility(final String abilityName) {
        ...
        final String lookupName = abilityName.equalsIgnoreCase("AirSurf") ? "AirScooter" : abilityName;
        return ABILITIES_BY_NAME.get(lookupName.toLowerCase());
    }
        ...
        if (clazz != null) {
            ...
            if (uuidMap != null) {
                for (final UUID uuid : uuidMap.keySet()) {
                    final Player uuidPlayer = Platform.players().getPlayer(uuid);
                    if (uuidPlayer != null) {
    ...
    public static void registerPluginAbilities(final JavaPlugin plugin, final String packageBase) {
        ...
        for (final CoreAbility coreAbil : loadedAbilities) {
            ...
            try {
                final AddonAbility addon = coreAbil instanceof AddonAbility ? (AddonAbility) coreAbil : null;
                if (addon != null) {
                    addon.load();
                }
                ...
                if (addon != null && coreAbil instanceof ComboAbility) {
                ...
                if (addon != null) {
                    final Permission bendingPlayerPerm = Platform.permissions().getPermission("bending.player");
                    Permission perm = Platform.permissions().getPermission("bending.ability." + coreAbil.getName());
                    if (perm == null) {
                        perm = new Permission("bending.ability." + coreAbil.getName());
                        Platform.permissions().addPermission(perm);
                        perm = Platform.permissions().getPermission("bending.ability." + coreAbil.getName());
                    }
                    // Reapply this on every registration. Bukkit retains dynamically
                    // created permissions across PK reloads, and the old conditional
                    // left those existing addon nodes permanently detached.
                    if (perm != null && bendingPlayerPerm != null) {
                        perm.addParent(bendingPlayerPerm, addon.isDefault());
                    }
                }
     ...
     */
    public static PKConfiguration getLanguageConfig() {
     ...
     */
    public void start() {
        if ((this.player == null && this.requiresPlayer()) || !this.isEnabled()) {
            return;
        }
        if (this.player != null) {
    ...
    public void remove() {
        ...
        if (this.player != null) {
            Platform.events().call(new AbilityEndEvent(this));
        }
        this.removed = true;
        AbilityRemovalSync.publish(this);

        final Map<UUID, Map<Integer, CoreAbility>> classMap = INSTANCES_BY_PLAYER.get(this.getClass());
        if (classMap != null && this.player != null) {
    ...
    public void recalculateAttributes() {
        if (recalculatingAttributes)
            return; //Stop recursion if an addon does something wrong, e.g. calls recalculateAttributes inside the event
    ...
    /**
     * @return the current PKConfiguration for the plugin
     */
