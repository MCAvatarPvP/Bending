...
import com.projectkorra.projectkorra.platform.mc.NamespacedKey;
import com.projectkorra.projectkorra.platform.mc.World;
import com.projectkorra.projectkorra.platform.mc.World.Environment;
...
public abstract class ElementalAbility extends CoreAbility {
    ...
    public static void addTags(Set<String> outputSet, List<String> configList) {
        ...
        iterator.forEachRemaining(next -> {
            if (next.startsWith("#")) {
                ...
                if (key != null) {
                    for (Material material : Platform.tags().values("blocks", key.getKey(), Material.class)) {
    ...
    public static boolean isIce(final Material material) {
        ...
        final Material configuredMaterial = Material.matchMaterial(configuredIce);
        return ICE_BLOCKS.contains(material.toString()) || material == configuredMaterial || WaterCosmetic.getCosmeticMats().contains(material);
    }
    ...
    public static boolean isMeltable(final Block block) {
        return isMeltableIce(block) || isSnow(block);
    }

    public static boolean isMeltableIce(final Block block) {
        return block != null && isIce(block) && (!WaterCosmetic.getCosmeticMats().contains(block.getType()) || TempBlock.isTempBlock(block));
    }
    ...
    public static boolean isTransparent(final Player player, final String abilityName, final Block block) {
        return !RaiseEarth.isBlockingAbility(block) && Arrays.asList(getTransparentMaterials()).contains(block.getType()) && !RegionProtection.isRegionProtected(player, block.getLocation(), CoreAbility.getAbility(abilityName));
    }
