...
public class EarthBlast extends EarthAbility {
    ...
    public void progress() {
        ...
        if (System.currentTimeMillis() - this.time >= this.interval) {
            ...
            if (this.isAtDestination) {
                this.remove();
                return;
            } else if (!this.isFocusedSourceValid()) {
                this.remove();
                return;
            }
            ...
            if (this.isAtDestination) {
                ...
                if (RaiseEarth.isBlockingAbility(this.location.getBlock())) {
                    this.remove();
                    return;
                }
    ...
    public void throwEarth() {
        ...
        } else if (this.location.getY() > player.getEyeLocation().getY() && GeneralMethods.isPassable(this.location.getBlock().getRelative(BlockFace.UP))) {
            this.firstDestination.subtract(0, 2, 0);
        } else if (GeneralMethods.isPassable(this.location.getBlock().getRelative(BlockFace.UP)) && GeneralMethods.isPassable(this.location.getBlock().getRelative(BlockFace.UP, 2))) {
    ...
    private boolean isFocusedSourceValid() {
        if (this.sourceBlock == null) {
            return false;
        }
        if (this.isEarthbendable(this.sourceBlock) || this.sourceBlock.getType() == Material.COBBLESTONE) {
            return true;
        }
        if (this.sourceType == Material.SAND && this.sourceBlock.getType() == Material.SANDSTONE) {
            return true;
        }
        if (this.sourceType == Material.RED_SAND && this.sourceBlock.getType() == Material.RED_SANDSTONE) {
            return true;
        }
        if (this.sourceType == Material.GRAVEL && this.sourceBlock.getType() == Material.STONE) {
            return true;
        }
        return false;
    }
