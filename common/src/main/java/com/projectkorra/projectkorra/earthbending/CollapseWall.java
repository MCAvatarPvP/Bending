...
public class CollapseWall extends EarthAbility {
    ...
    public CollapseWall(final Player player) {
        ...
        for (final Block block : GeneralMethods.getBlocksAroundPoint(this.location, this.radius)) {
            if (RaiseEarth.blockInWallAffectedBlocks(block) && this.isEarthbendable(block) && !this.blocks.containsKey(block) && block.getY() >= this.location.getBlockY()) {
    ...
    private void getAffectedBlocks(final Block block) {
        ...
        for (int i = 1; i <= this.height; i++) {
            ...
            if (RaiseEarth.blockInWallAffectedBlocks(blocki) && this.isEarthbendable(blocki)) {
                baseBlock = blocki;
