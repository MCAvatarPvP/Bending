...
public class Torrent extends WaterAbility {
    ...
    private void freeze() {
        ...
        for (final Block block : ice) {
            if (isTransparent(this.player, block) && !isIce(block)) {
                ...
                final TempBlock tblock = new TempBlock(block, getIceData(), this);
        ...
        if (System.currentTimeMillis() > this.time + this.interval) {
            ...
            if (this.sourceSelected) {
                ...
                if (this.player.isSneaking()) {
                    ...
                    this.source = new TempBlock(this.sourceBlock, isCauldron(this.sourceBlock) ? this.sourceBlock.getBlockData() : Material.WATER.createBlockData(), this);
            ...
            if (this.settingUp) {
                ...
                if (this.location.distanceSquared(setup) <= 1) {
                    ...
                    this.source = new TempBlock(this.location.getBlock(), isCauldron(this.location.getBlock()) ? this.location.getBlock().getBlockData() : Material.WATER.createBlockData(), this);
                }
            }
    ...
    private boolean launch() {
        ...
        if (this.launchedBlocks.isEmpty()) {
            ...
            for (double theta = this.startAngle; theta < this.angle + this.startAngle; theta += 20) {
                ...
                if (!doneBlocks.contains(block) && !GeneralMethods.isRegionProtectedFromBuild(this, blockloc)) {
                    if (isTransparent(this.player, block)) {
                        this.launchedBlocks.add(new TempBlock(block, Material.WATER.createBlockData(), this));
        ...
        for (int i = 0; i < this.launchedBlocks.size(); i++) {
            ...
            if (i == this.launchedBlocks.size() - 1) {
                ...
                newBlocks.add(block);
                for (final Entity entity : this.getTorrentCollider(block.getBlock()).getEntities(found -> !affectedEntities.contains(found))) {
                    if (i == 0) {
        ...
        for (double theta = this.startAngle; theta < this.angle + this.startAngle; theta += 20) {
            ...
            if (!doneBlocks.contains(block)) {
                if (isTransparent(this.player, block)) {
                    ...
                    doneBlocks.add(block);
                    for (final Entity entity : this.getTorrentCollider(block).getEntities(found -> !affectedEntities.contains(found))) {
                        this.deflect(entity);
                        affectedEntities.add(entity);
                    }
                }
    ...
    private AABB getTorrentCollider(final Block block) {
        final Location center = block.getLocation().clone().add(0.5, 0.5, 0.5);
        return new AABB(center, 0.75, 0.9, 0.75);
    }
