            ...
            if (this.state == AbilityState.ICE_BULLET_FORMING) {
                if (timeDiff < 1000 * this.animationSpeed) {
                    ...
                    for (double i = 0; i < this.radius; i++) {
                        this.drawWaterCircle(this.player.getEyeLocation().clone().add(0, i, 0), 360, 5, this.radius - i, getIceMaterial(), getIceData());
                        this.drawWaterCircle(this.player.getEyeLocation().clone().add(0, -i, 0), 360, 5, this.radius - i, getIceMaterial(), getIceData());
                    }
                }
                ...
                if (timeDiff < this.shootTime) {
                    if (this.rightClicks > 0 || this.leftClicks > 0) {
                        ...
                        final ParticleStream fs = new ParticleStream(this.player, this, vec, loc, this.range, this.speed);
