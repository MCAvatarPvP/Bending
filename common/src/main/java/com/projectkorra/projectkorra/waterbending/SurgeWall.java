...
public class SurgeWall extends WaterAbility {
    ...
    @Attribute("Wall" + Attribute.RADIUS)
    @DayNightFactor
    private double radius;
    @Attribute("Wall" + Attribute.RANGE)
    @DayNightFactor
    ...
    public void progress() {
        ...
        if (System.currentTimeMillis() - this.time >= this.interval) {
            ...
            if (this.forming) {
                ...
                final Location targetLoc = GeneralMethods.getTargetedLocation(this.player, (int) this.range, false, false, Material.WATER, getIceMaterial());
    ...
    private void addWallBlock(final Block block) {
        if (this.frozen) {
            tempBlocks.put(block, new TempBlock(block, getIceData(), this));
        } else {
            tempBlocks.put(block, new TempBlock(block, Material.WATER.createBlockData(), this));
        }
    }
