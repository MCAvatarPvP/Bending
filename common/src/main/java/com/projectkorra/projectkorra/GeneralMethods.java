...
public class GeneralMethods {
    ...
    public static Element getParentElement(Element element) {
        if (element instanceof Element.SubElement) return ((Element.SubElement) element).getParentElement();
        return element;
    }
    ...
    public static boolean checkDiagonalWall(final Location location, final Vector direction) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            xyzsolid[i] = isSolid(location.getBlock().getRelative(face));
        }
    ...
    public static boolean isOnGround(Entity entity) {
        ...
        return !underFeetAABB.getBlocks(block -> !isPassable(block)).isEmpty();
    }
    ...
    public static Plugin getItems() {
        if (hasItems()) {
            return Platform.plugins().getPlugin("ProjectKorraItems");
        }
        return null;
    }
    ...
    public static Plugin getProbending() {
        if (hasProbending()) {
            return Platform.plugins().getPlugin("Probending");
        }
        return null;
    }
    ...
    public static Plugin getRPG() {
        if (hasRPG()) {
            return Platform.plugins().getPlugin("ProjectKorraRPG");
        }
        return null;
    }
    ...
    public static Plugin getSpirits() {
        if (hasSpirits()) {
            return Platform.plugins().getPlugin("ProjectKorraSpirits");
        }
        return null;
    }
    ...
    public static boolean hasItems() {
        return Platform.plugins().getPlugin("ProjectKorraItems") != null;
    }

    public static boolean hasProbending() {
        return Platform.plugins().getPlugin("Probending") != null;
    }

    public static boolean hasRPG() {
        return Platform.plugins().getPlugin("ProjectKorraRPG") != null;
    }

    public static boolean hasSpirits() {
        return Platform.plugins().getPlugin("ProjectKorraSpirits") != null;
    }
    ...
    public static boolean isInteractable(final Material material) {
        if (material == LIGHT) return false;
    ...
    public static boolean isSolid(final Block block) {
        return RaiseEarth.isBlockingAbility(block) || block.isSolid();
    }
    ...
    public static boolean isPassable(final Block block) {
        return block.isPassable() && !RaiseEarth.isBlockingAbility(block);
    }

    public static boolean isTransparent(final Block block) {
        return !RaiseEarth.isBlockingAbility(block) && isTransparent(block.getType());
    }
    ...
    public static void reloadPlugin(final CommandSender sender) {
        ...
        ConfigManager.waterCosmeticsConfig.reload();
        ...
        Arrays.stream(Element.getElements()).forEach(e -> {
            e.setColor(null);
            e.setSubColor(null);
        }); //Load colors from config again
        Arrays.stream(Element.getSubElements()).forEach(e -> {
            e.setColor(null);
            e.setSubColor(null);
        }); //Same for subs
        ...
        ProjectKorra.plugin.revertChecker = Platform.scheduler().runTimerAsync(new RevertChecker(ProjectKorra.plugin), 0, 200);
        ...
        ProjectKorra.log.info("Registered " + CoreAbility.getAbilities().size() + " abilities");
        reloadAddonPlugins();  //Register all addons and addon listeners again
        ComboManager.registerCombos();
        // fallDamageConfig.yml resolves class names, so it must be loaded only
        // after core, embedded, and external addon abilities are registered.
        ...
        if (!DBConnection.isOpen()) {
            ProjectKorra.log.severe("Unable to enable ProjectKorra because no storage adapter could be opened");
            return;
        }
        Manager.shutdown();
        Manager.startup();
        ...
        if (plugin.updater != null) plugin.updater.checkUpdate();
        ProjectKorra.log.info("Reload complete");
    }

    public static void reloadAddonPlugins() {
        // Registration order is significant: addon abilities intentionally replace
        // same-named core entries. Replaying this list backwards made the hidden
        // legacy LavaSurge overwrite ProjectAddons' bindable LavaSurge.
        for (final String entry : new ArrayList<>(CoreAbility.getAddonPlugins())) {
            final String[] split = entry.split("::");
            if (ProjectKorra.plugin.getName().equals(split[0])) {
                CoreAbility.registerPluginAbilities(ProjectKorra.plugin, split[1]);
                continue;
            }
            if (Platform.plugins().isPluginEnabled(split[0])) {
                final Object plugin = Platform.plugins().getPlugin(split[0]);
                if (plugin instanceof JavaPlugin javaPlugin) {
                    CoreAbility.registerPluginAbilities(javaPlugin, split[1]);
                }
            } else {
                CoreAbility.getAddonPlugins().remove(entry);
            }
        }
    }
    ...
    public static boolean runDebug() {
        ...
        f.add("Bukkit Version: " + Platform.server().version());
        f.add("");
        ...
        for (String pluginName : RegionProtection.getActiveProtections().keySet()) {
            Plugin plugin = Platform.plugins().getPlugin(pluginName);
            if (plugin != null && plugin.isEnabled()) {
                f.add("- " + plugin.getName() + " v" + plugin.getDescription().getVersion());
            }
        }

        Plugin papi = Platform.plugins().getPlugin("PlaceholderAPI");
        ...
        for (final Plugin plugin : Platform.server().<Plugin>plugins()) {
    ...
    public static void playSound(Location loc, Sound defaultSound, String sound, float volume, float pitch) {
        ...
        String selected = firstLegacyConfigAlternative(sound);
        if (selected == null || selected.isBlank()) return;

        // Namespaced and dotted vanilla/custom keys should use the string sound path.
        // Legacy configs often contain alternatives like
        // "minecraft:entity.breeze.slide # minecraft:entity.breeze.idle_ground, minecraft:entity.horse.breathe".
        // Passing that whole value to Fabric makes Identifier validation fail, so select one key first.
        if (selected.contains(":") || selected.contains(".")) {
            loc.getWorld().playSound(loc, selected, volume, pitch);
            return;
        }

        Sound s = defaultSound;
        try {
            s = Sound.valueOf(selected);
        } catch (final IllegalArgumentException exception) {
            ProjectKorra.log.warning("Sound name '" + selected + "' is not valid.");
        } finally {
            loc.getWorld().playSound(loc, s, volume, pitch);
        }
    }

    public static String firstLegacyConfigAlternative(final String raw) {
    ...
    public static void stopPlugin() {
        ProjectKorra.plugin.getServer().getPluginManager().disablePlugin(ProjectKorra.plugin);
    }
    ...
    public static void setVelocity(Ability ability, Entity entity, Vector vector) {
        ...
        // Keep velocity-triggered fall protection loader-neutral. The old hook
        // lived only in the Bukkit listener, so fallDamageConfig.yml could never
        // work on Fabric or in exact client prediction.
        if (event.getAffected() instanceof Player affectedPlayer && velocity.getY() >= 0) {
            BendingPlayer affectedBender = BendingPlayer.getBendingPlayer(affectedPlayer);
            if (affectedBender != null) {
                if (affectedBender.hasElement(Element.AIR) || affectedBender.hasElement(Element.EARTH)) {
                    affectedPlayer.setFallDistance(0);
                }
                if (event.getAbility() instanceof CoreAbility coreAbility
                        && FallHandler.shouldStopFallFromVelocity(affectedPlayer, coreAbility)) {
                    FallHandler.stopFall(affectedPlayer);
                }
            }
        }
    ...
    public static int getMCVersion() {
        String version = Platform.server().minecraftVersion();
