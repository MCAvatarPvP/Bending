...
import com.projectkorra.projectkorra.platform.mc.potion.PotionEffectTypeWrapper;
...
public class AvatarState extends AvatarAbility {
    ...
    public AvatarState(final Player player) {
        ...
        PKConfiguration avatarConfig = ConfigManager.avatarStateConfig.get();
        PKConfiguration legacyConfig = ConfigManager.defaultConfig.get();
        PKConfigurationSection potionSection = avatarConfig.getConfigurationSection("PotionEffects");
        if (potionSection == null) {
            potionSection = legacyConfig.getConfigurationSection("Abilities.Avatar.AvatarState.PotionEffects");
        }
        if (potionSection != null) {
        ...
        // Runtime state settings live in Styles/avatarstate.yml.  Keep legacy
        // config.yml paths as fallbacks only for old installations.
        this.duration = avatarConfig.getLong("AvatarState.Duration", legacyConfig.getLong("Abilities.Avatar.AvatarState.Duration"));
        this.cooldown = avatarConfig.getLong("AvatarState.Cooldown", legacyConfig.getLong("Abilities.Avatar.AvatarState.Cooldown"));
        this.showParticles = avatarConfig.getBoolean("AvatarState.ShowParticles", legacyConfig.getBoolean("Abilities.Avatar.AvatarState.ShowParticles"));
        this.playSound = avatarConfig.getBoolean("AvatarState.PlaySound", legacyConfig.getBoolean("Abilities.Avatar.AvatarState.PlaySound"));
        this.glow = avatarConfig.getBoolean("AvatarState.GlowEnabled", legacyConfig.getBoolean("Abilities.Avatar.AvatarState.GlowEnabled"));
    ...
    public static double getValue(final double value) {
        final double factor = ConfigManager.getConfig().getDouble("Abilities.Avatar.AvatarState.PowerMultiplier");
        return factor * value;
    }
