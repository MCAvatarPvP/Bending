...
public class Commands {
    ...
    public static boolean dispatch(final CommandSender sender,
        ...
        if (!Arrays.asList(commandaliases).contains(label.toLowerCase())) return false;
        if (args.length > 0) {
        ...
        PKCommand help = PKCommand.instances.get("help");
        if (help != null) help.execute(sender, new ArrayList<>());
        return true;
    }
    ...
    static void initializeCommands() {
        ...
        new WhoCommand();
        new CooldownCommand();
        new ComboHelpCommand();
        new TempCommand();
        new ColliderCommand();
        new CollisionEffectCommand();
        new ViewDistanceCommand();
        new DetailedActionBarCommand();
        new OldScooterCommand();
    }
    ...
    private void init() {
        ...
        final CommandExecutor exe = (s, c, label, args) -> dispatch(s, label, args);
