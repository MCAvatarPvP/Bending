...
public abstract class PKCommand implements SubCommand {
    ...
    public List<Player> getOnlinePlayers(final CommandSender sender) {
        return Platform.players().<Player>onlinePlayers().stream().filter(p -> !(sender instanceof Player) || ((Player) sender).canSee(p)).collect(Collectors.toList());
    }
    ...
    public List<String> getOnlinePlayerNames(final CommandSender sender) {
        return Platform.players().<Player>onlinePlayers().stream().filter(p -> !(sender instanceof Player) || ((Player) sender).canSee(p)).map(Player::getName).collect(Collectors.toList());
    }
    ...
    public CompletableFuture<OfflinePlayer> getPlayer(String name) {
        ...
        if (Platform.players().getPlayer(name) != null)
            return CompletableFuture.completedFuture(Platform.players().getPlayer(name));

        Platform.scheduler().runAsync(() -> {
            OfflinePlayer player = Platform.players().getOfflinePlayer(name);
            Platform.scheduler().runNow(() -> future.complete(player)); //Complete in a sync thread
        });
