...
public class AddCommand extends PKCommand {
                        ...
                        if (online) {
                            for (final SubElement sub : Element.getAllSubElements()) {
                                if (bPlayer.hasElement(sub.getParentElement()) && ((BendingPlayer) bPlayer).hasSubElementPermission(sub)) {
                                    PlayerChangeSubElementEvent subEvent = new PlayerChangeSubElementEvent(sender, target, sub, PlayerChangeSubElementEvent.Result.ADD);
                                    Platform.events().call(subEvent);
                                    if (subEvent.isCancelled())
                                        continue; // if the event is cancelled, don't add the subelement.
                ...
                if (Arrays.asList(Element.getAllElements()).contains(e)) {
                    ...
                    if (!(sender instanceof Player) || !((Player) sender).equals(target)) {
                        if (e != Element.AIR && e != Element.EARTH && e != Element.BLUE_FIRE) {
                            ...
                            if (online)
                                ChatUtil.sendBrandingMessage((Player) target, color + this.addedCFW.replace("{element}", e.toString() + e.getType().getBender()));
                        } else {
                            ...
                            if (online)
                                ChatUtil.sendBrandingMessage((Player) target, color + this.addedAE.replace("{element}", e.toString() + e.getType().getBender()));
                        }
                    } else {
                    ...
                    PlayerChangeSubElementEvent event = new PlayerChangeSubElementEvent(sender, target, sub, PlayerChangeSubElementEvent.Result.ADD);
                    Platform.events().call(event);
                    ...
                    if (!(sender instanceof Player) || !((Player) sender).equals(target)) {
                        ...
                        if (e != Element.AIR && e != Element.EARTH) {
                            if (online)
                                ChatUtil.sendBrandingMessage((Player) target, color + this.addedCFW.replace("{element}", sub.toString() + sub.getType().getBender()));
                        } else {
                            if (online)
                                ChatUtil.sendBrandingMessage((Player) target, color + this.addedAE.replace("{element}", sub.toString() + sub.getType().getBender()));
                        }
                    }
                    bPlayer.saveSubElements();
                    return;
    ...
    protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
        ...
        if (args.size() == 0) {
            ...
            for (final Player p : Platform.players().<Player>onlinePlayers()) {
                l.add(p.getName());
            }
        }
        return l;
