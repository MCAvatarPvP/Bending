        ...
        BendingPlayer.getOrLoadOfflineAsync(target).thenAccept(bPlayer -> {
            ...
            if (element instanceof SubElement) {
                ...
                if (!(sender instanceof Player) || !((Player) sender).equals(target)) {
                    ...
                    if (online)
                        ChatUtil.sendBrandingMessage((Player) target, color + this.chosenCFW.replace("{element}", sub.getName() + sub.getType().getBender()));
                }
                bPlayer.saveSubElements();
                if (online) {
                    ((BendingPlayer) bPlayer).removeUnusableAbilities();
                }
                Platform.events().call(new PlayerChangeSubElementEvent(sender, target, sub, PlayerChangeSubElementEvent.Result.CHOOSE));
            } else {
                ...
                if (!(sender instanceof Player) || !((Player) sender).equals(target)) {
                    ...
                    if (element != Element.AIR && element != Element.EARTH) {
                        if (online)
                            ChatUtil.sendBrandingMessage((Player) target, color + this.chosenCFW.replace("{element}", element.getName() + element.getType().getBender()));
                    } else {
                        if (online)
                            ChatUtil.sendBrandingMessage((Player) target, color + this.chosenAE.replace("{element}", element.getName() + element.getType().getBender()));
                    }
                }
