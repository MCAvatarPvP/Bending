...
import com.projectkorra.projectkorra.platform.chat.HoverEvent;
...
public class CooldownCommand extends PKCommand {
    ...
    public void execute(CommandSender sender, List<String> list) {
        ...
        OfflinePlayer oPlayer = list.size() == 1 ? (Player) sender : Platform.players().getOfflinePlayer(list.get(1));
    ...
    protected List<String> getTabCompletion(CommandSender sender, List<String> args) {
        ...
        if (args.size() < 2) return getOnlinePlayerNames(sender);
            ...
            list.add("ALL");
            return list;
        } else if (Arrays.asList(new String[]{"set", "s"}).contains(args.get(0)))
            return Lists.newArrayList("60s", "30m", "2h", "30d");
