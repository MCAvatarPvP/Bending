            ...
            if (e == null) { //The first argument must be a playername instead
                getPlayer(args.get(0)).thenAccept(player -> {
                    ...
                    BendingPlayer.getOrLoadOfflineAsync(player).thenAccept(bPlayer -> {
                        ...
                        if (online) {
                            ...
                            Platform.events().call(new PlayerChangeElementEvent(sender, (Player) player, null, Result.REMOVE));
                        }
                    }).exceptionally(ex -> {
                        ex.printStackTrace();
                        return null;
                ...
                if (e instanceof SubElement) { //If it's a subelement
                    if (senderBPlayer.hasElement(e)) {
                        ...
                        Platform.events().call(new PlayerChangeSubElementEvent(sender, player, (SubElement) e, PlayerChangeSubElementEvent.Result.REMOVE));
                    } else {
                ...
                if (senderBPlayer.hasElement(e)) {
                    ...
                    Platform.events().call(new PlayerChangeElementEvent(sender, (Player) sender, e, Result.REMOVE));
                    return;
                } else {
                    ChatUtil.sendBrandingMessage(sender, ChatColor.RED + this.wrongElementSelf);
                }
        ...
        getPlayer(args.get(0)).thenAccept(player -> {
            ...
            BendingPlayer.getOrLoadOfflineAsync(player).thenAccept(bPlayer -> {
                ...
                if (online) {
                    ...
                    Platform.events().call(new PlayerChangeElementEvent(sender, (Player) player, element, Result.REMOVE));
                }
            }).exceptionally(ex -> {
                ex.printStackTrace();
                return null;
