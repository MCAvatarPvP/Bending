...
public class LightManager {
    ...
    private void precomputeLightData() {
        BlockData lightData = Platform.server().createBlockData(Material.valueOf("LIGHT"));
    ...
    private void sendLightChange(Location location, int brightness, Collection<? extends Player> observers) {
        ...
        double maxDistanceSquared = Math.pow(Platform.server().viewDistance() * 16, 2);
    ...
    public static class LightBuilder {
        ...
        private Collection<? extends Player> observers = Platform.players().<Player>onlinePlayers(); // default to all players
