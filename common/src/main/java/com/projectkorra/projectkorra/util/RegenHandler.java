...
import com.projectkorra.projectkorra.platform.mc.event.entity.EntityRegainHealthEvent;
...
public class RegenHandler implements Runnable {
    ...
    public void run() {
        ...
        for (Player player : Platform.players().<Player>onlinePlayers()) {
            ...
            if (bendingPlayer == null || now - bendingPlayer.getLastHealthRegen() < regenInterval(bendingPlayer))
                continue;
