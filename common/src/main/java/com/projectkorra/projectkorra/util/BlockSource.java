...
public class BlockSource {
    ...
    private static PKConfiguration config = ConfigManager.defaultConfig.get();
    ...
    public static void update(final Player player, final ClickType clickType) {
        ...
        if (coreAbil instanceof WaterAbility) {
            removeSource(player, BlockSourceType.WATER, clickType);
            removeSource(player, BlockSourceType.ICE, clickType);
            removeSource(player, BlockSourceType.PLANT, clickType);
            removeSource(player, BlockSourceType.SNOW, clickType);
            removeSource(player, BlockSourceType.MUD, clickType);
            ...
            if (waterBlock != null) {
                ...
                if (WaterAbility.isIcebendable(player, waterBlock, false)) {
            ...
            removeSource(player, BlockSourceType.EARTH, clickType);
            removeSource(player, BlockSourceType.METAL, clickType);
            removeSource(player, BlockSourceType.MUD, clickType);
            removeSource(player, BlockSourceType.LAVA, clickType);
    ...
    public static Block getWaterSourceBlock(final Player player, final double range, final ClickType clickType, final boolean allowWater, final boolean allowIce, final boolean allowPlant, final boolean allowSnow, final boolean allowWaterBottles) {
        ...
        if (sourceBlock != null && !ElementalAbility.isAir(sourceBlock.getType()) && WaterAbility.isWaterbendable(player, null, sourceBlock)) {
    ...
    private static boolean isStillAValidSource(final BlockSourceInformation info, final double range, final ClickType clickType) {
        if (info == null || info.getBlock() == null) {
            ...
            return false;
        } else if (info.getSourceType() == BlockSourceType.ICE && !WaterAbility.isIcebendable(info.getPlayer(), info.getBlock(), false)) {
            return false;
