...
import com.projectkorra.projectkorra.platform.mc.entity.TNTPrimed;
...
public class FireBlastCharged extends FireAbility {
    ...
    public FireBlastCharged(final Player player) {
        ...
        if (canChargeInWater || canPassThroughWater(player.getEyeLocation().getBlock()) || !player.getEyeLocation().getBlock().isLiquid()) {
            this.start();
        }
    }
            ...
            if (this.canDamageBlocks && this.explosionRadius > 0) {
                for (final Block block : GeneralMethods.getBlocksAroundPoint(this.location, this.explosionRadius)) {
                    if (explosionPower >= (double) block.getType().getBlastResistance()) {
                        new TempBlock(block, Material.AIR.createBlockData(), damagedRevertTime, this);
                    }
                }
            }
    ...
    public void progress() {
        ...
        if (System.currentTimeMillis() > this.time + this.interval) {
            ...
            for (double i = 0; i < collisionRadius; i += collisionRadius / 2) {
                ...
                if (TempBlock.isTempBlock(this.location.getBlock()) && TempBlock.get(this.location.getBlock()).getAbility().orElse(null) instanceof EarthSmash) {
                    // Let the ability collision system resolve this against EarthSmash.
                } else if (GeneralMethods.isSolid(this.location.getBlock())) {
                    this.explode();
                    return;
                } else if (this.location.getBlock().isLiquid() && !canPassThroughWater(this.location.getBlock())) {
                    this.remove();
                    return;
                }
