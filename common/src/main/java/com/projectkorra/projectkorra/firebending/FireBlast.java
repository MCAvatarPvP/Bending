...
import com.projectkorra.projectkorra.platform.mc.block.BlastFurnace;
import com.projectkorra.projectkorra.platform.mc.block.Block;
import com.projectkorra.projectkorra.platform.mc.block.Furnace;
import com.projectkorra.projectkorra.platform.mc.block.Smoker;
import com.projectkorra.projectkorra.platform.mc.block.data.type.Campfire;
...
public class FireBlast extends FireAbility {
    ...
    public FireBlast(final Location location, final Vector direction, final Player player, final double damage, final List<Block> safeBlocks) {
        ...
        if (location.getBlock().isLiquid() && !canPassThroughWater(location.getBlock())) {
            return;
        }
    ...
    public FireBlast(final Player player) {
        ...
        if (this.bPlayer.isOnCooldown("FireBlast")) {
            return;
        } else if (((mat == Material.WATER && !canPassThroughWater(player.getEyeLocation().getBlock())) || !canGoThroughLava && mat == Material.LAVA) || FireBlastCharged.isCharging(player)) {
            return;
        }
    ...
    public boolean checkLocation(Block block) {
        if ((block.getType() == Material.WATER && !canPassThroughWater(block)) || !canGoThroughLava && block.getType() == Material.LAVA) {
            this.remove();
            return false;
        }
        if (TempBlock.isTempBlock(block) && TempBlock.get(block).getAbility().orElse(null) instanceof EarthSmash) {
            return true;
        }
        ...
        if (!GeneralMethods.isPassable(block)) {
            if (block.getType() == Material.FURNACE && this.powerFurnace) {
                ...
                blastF.update();
            } else if (block.getBlockData() instanceof Campfire) {
