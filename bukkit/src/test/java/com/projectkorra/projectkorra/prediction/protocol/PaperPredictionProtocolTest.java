...
import com.projectkorra.projectkorra.prediction.action.PredictionEchoPolicy;
...
class PaperPredictionProtocolTest {
    ...
    @Test
    void abilityRemovalCarriesExternalAndPredictionRejectionCausesSeparately() {
        UUID owner = UUID.randomUUID();
        PaperPredictionProtocol.Reader reader = new PaperPredictionProtocol.Reader(
    ...
    @Test
    void earthSmashTransferCarriesExactStateLocationTimingAndShape() {
        UUID owner = UUID.randomUUID();
        EarthSmash.PredictionTransfer transfer = new EarthSmash.PredictionTransfer(
                "paper-world-name", 1.25, 65.5, -4.75,
                true, 10.0, 70.0, -12.0, "GRABBED", 5.5,
                3, 8, 29L, 1_250L, 500L, 40L,
        ...
        assertTrue(reader.bool());
        assertEquals(17, reader.varInt());
        assertEquals(1.25, reader.f64());
        assertEquals(65.5, reader.f64());
        assertEquals(-4.75, reader.f64());
        assertTrue(reader.bool());
        assertEquals(10.0, reader.f64());
        assertEquals(70.0, reader.f64());
        assertEquals(-12.0, reader.f64());
        assertEquals("GRABBED", reader.string(64));
        assertEquals(5.5, reader.f64());
        assertEquals(3, reader.i32());
        assertEquals(8, reader.i32());
        assertEquals(29L, reader.varLong());
        assertEquals(1_250L, reader.varLong());
        assertEquals(500L, reader.varLong());
        assertEquals(40L, reader.varLong());
        assertEquals(1, reader.varInt());
        assertEquals(-1, reader.i32());
        assertEquals(0, reader.i32());
        assertEquals(1, reader.i32());
        assertEquals("minecraft:stone[axis=y]",
                reader.string(PaperPredictionProtocol.MAX_BLOCK_STATE_CHARACTERS));
        reader.finished();
    }
    ...
    void tempBlockReceiptCarriesStableLayerOwnerAndHiddenState() {
        ...
        assertEquals("LIFTED", reader.string(64));
    ...
    void boundedTempBlockPageAlwaysFitsPluginMessageLimit() {
        ...
        String maximumEffectState = "界".repeat(64);
        UUID owner = UUID.randomUUID();
        List<PaperPredictionProtocol.TempBlockOp> page = java.util.stream.LongStream.range(0, 4)
                .mapToObj(layer -> new PaperPredictionProtocol.TempBlockOp(
                        ...
                        Long.MAX_VALUE, Long.MAX_VALUE, maximumAbility, maximumEffectState, Long.MAX_VALUE,
