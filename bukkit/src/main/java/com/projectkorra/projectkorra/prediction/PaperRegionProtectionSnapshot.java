...
final class PaperRegionProtectionSnapshot {
    private static final int SPATIAL_RADIUS = 72;

    private PaperRegionProtectionSnapshot() {
    }

    static RegionProtectionAuthority.Snapshot build(final Player player,
    ...
    static RegionProtectionAuthority.Snapshot currentPoint(final Player player,
                                                             final Collection<String> abilityNames) {
    ...
    static RegionProtectionAuthority.Snapshot spatial(final Player player,
                                                        final Collection<String> abilityNames) {
        if (player == null) return RegionProtectionAuthority.Snapshot.empty();
        final List<String> abilities = RegionProtectionAuthority.normalizedAbilities(abilityNames);
        final String world = player.getWorld().getKey().toString();
    ...
    static List<String> relevantAbilities(final Player player, final Collection<String> bound) {
