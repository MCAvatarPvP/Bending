package com.projectkorra.projectkorra.prediction.protocol;
...
public final class PaperPredictionProtocol {
    ...
    public static final String HIT_CLAIM = "projectkorra:hit_claim";
    public static final String SNAPSHOT = "projectkorra:server_snapshot";
    public static final String WORLD_STATE = "projectkorra:world_state";
    public static final String NATIVE_ACTION = "projectkorra:native_action";
    public static final String STATE = "projectkorra:player_state";
    public static final String CONFIG_CHUNK = "projectkorra:config_chunk";
    public static final String RECONCILE = "projectkorra:reconcile";
    public static final String TEMP_BLOCKS = "projectkorra:temp_blocks";
    ...
    public static Ready readReady(byte[] data) {
        Reader reader = new Reader(data);
    ...
    public static InputVeto readInputVeto(byte[] data) {
        Reader reader = new Reader(data);
    ...
    public static HitClaim readHitClaim(byte[] data) {
        Reader reader = new Reader(data);
    ...
    public static byte[] snapshot(UUID session, long serverTick, long serverNowMillis, long epoch, int rewindTicks,
    ...
    public static byte[] worldState(UUID session, long worldGeneration, String worldIdentity) {
        return new Writer().uuid(session).varLong(worldGeneration).string(worldIdentity, 128).bytes();
    }

    public static byte[] configChunk(UUID session, long epoch, int index, int count, List<ConfigEntry> config) {
        Writer out = new Writer();
    ...
    public static int configEntrySize(ConfigEntry entry) {
        Writer out = new Writer();
    ...
    public static byte[] nativeAction(UUID session, long sequence, long clientSequence,
        ...
        return new Writer().uuid(session).varLong(sequence).varLong(clientSequence)
                .i64(serverTick).enumeration(kind)
    ...
    public static byte[] velocityOwnerV2(long serverTick, long actionSequence, int impulseOrdinal,
    ...
    public static byte[] abilityStateOwner(long serverTick, long actionSequence, int mutationOrdinal,
                                    UUID abilityOwner, UUID target, String ability,
    ...
    public static byte[] tempFallingBlockPrepare(long serverTick, long actionSequence, int spawnOrdinal,
    ...
    public static byte[] directBlock(long serverTick, long actionSequence, int mutationOrdinal,
                              UUID abilityOwner, String ability, String world,
    ...
    public static byte[] abilityTransfer(final UUID player, final long actionSequence,
        ...
        final Writer out = new Writer().uuid(player).varLong(actionSequence)
                ...
                .bool(transfer.hasDestination()).f64(transfer.destinationX())
                .f64(transfer.destinationY()).f64(transfer.destinationZ())
                .string(transfer.state(), 64).f64(transfer.grabbedDistance())
                .i32(transfer.animationCounter()).i32(transfer.progressCounter())
                .varLong(transfer.predictionFrame())
                .varLong(transfer.elapsedMillis()).varLong(transfer.flightElapsedMillis())
                .varLong(transfer.delayElapsedMillis()).varInt(transfer.blocks().size());
        for (final EarthSmash.PredictionBlock block : transfer.blocks()) {
            out.i32(block.x()).i32(block.y()).i32(block.z())
    ...
    public record HitClaim(UUID session, long clientSequence, long serverSequence, long clientTick,
                    UUID target, int entityId, String ability, double x, double y, double z) {
    }

    public record TempBlockOp(TempOperation operation, String world, int x, int y, int z, String material,
                       long revertAtMillis, long actionSequence, String effectAbility,
                       String effectState, long effectStep, int effectOrdinal, long layerId, long revision,
